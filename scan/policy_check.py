...
# 字符串匹配规则
def str_match(value, proposed_val):
    # 对账户名进行排序
...
def get_policy_info(scan_id, basename):
    ...
    # 将文件中的信息读出来，存入字典
    dic = {}
    with codecs.open('policy.ini', 'r+', 'utf-16') as fd:
    ...
    for result in results:
        ...
        # 更新数据库
        cursor.execute('update config set checked_val = ? where id = ?', (value, result[0],))
        cursor.execute('update config set eval_result = ? where id = ?', (eval_result, result[0],))
